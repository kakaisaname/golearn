...
type Reader struct { ... }       NewReaderSize------------------------------------NewReader、NewReaderS
...
// Peek 返回缓存的一个切片，该切片引用缓存中前 n 个字节的数据，-----------------------------------Peek
...
func (b *Reader) ReadSlice(delim byte) (line []byte, err error)ize-------------------------

// ReadLine 是一个低水平的行读取原语，大多数情况下，应该使用 ----------------------------------------------不建议使用
// ReadBytes('\n') 或 ReadString('\n')，或者使用一个 Scanner。
//
...
func main() {
	...
	buf := bufio.NewReaderSize(sr, 0)  //size 小于 16 被设置为16
	b := make([]byte, 10)

	fmt.Println(buf.Buffered()) // 0    缓存中未读取的数据为 0
	s, _ := buf.Peek(5)               //返回缓存的一个切片   Peek了一下后 缓存中未读取的数据长度为16了
	s[0], s[1], s[2] = 'a', 'b', 'c'
...
func main() {
	...
	onComma := func(data []byte, atEOF bool) (advance int, token []byte, err error) {
		for i := 0; i < len(data); i++ {
			if data[i] == ',' {
				return i + 1, data[:i], nil  //data[:i]  截取非 ，数据
			}
		}
		if atEOF {           //-------------------------------------****************
			// 告诉 Scanner 扫描结束。
